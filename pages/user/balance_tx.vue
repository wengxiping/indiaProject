	  ...
	  <view class="content-list">
		  <view class="content-item">
			  ...
			  <view class="content-item-group">
				  <view class="group-button-default" v-if="Object.keys(bankItem).length == 0 || money<=0">确定提现</view>
				   <view class="group-button" @tap="setMyoutmoneyFunc()" v-else>确定提现</view>
		...
		methods:{
			...
			changeKeyboard(val)
			{
				...
				if(this.messageValue.length == this.maxLength){
					this.show = false
					uni.$u.api.setMyoutmoney({bcid:this.bankItem.bcid,money:this.money,zfpd:this.messageValue}).then(ret=>{
						uni.$u.toast(ret.msg);
						if(ret.code == 1)
						{
							this.money=""
		...
		.content-item
		{
			...
			.content-item-content-middle
			{
				...
				.content-item-content-middle-content{
					...
					.iput-number
					{
						width: 100%;height: 100%;color: #000000;font-size: 30rpx;text-shadow: 20rpx;
					}
				}
